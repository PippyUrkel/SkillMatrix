                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {currentCourse.complementarySkills.map((skill) => (
                        <span key={skill} className="text-sm text-[#6B7280]">
                          + {skill}
                        </span>
                      ))}
                    </div>
                ...
                <div className="space-y-4">
                  {currentCourse.checkpoints.length > 0 ? (
                      ...
                      return (
                        <div key={checkpoint.id} className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                          ...
                          {showResult && (
                            <div className={cn(
                              'mt-3 p-3 rounded-lg',
                              isCorrect ? 'bg-[rgba(46,233,168,0.1)]' : 'bg-[rgba(255,68,68,0.1)]'
                            )}>
                              <p className={isCorrect ? 'text-[#2EE9A8]' : 'text-[#FF4444]'}>
                                {isCorrect ? '✓ Correct!' : '✗ Incorrect'}
                              </p>
                              {!isCorrect && (
                                <p className="text-[#A7B4AD] text-sm mt-1">{checkpoint.explanation}</p>
                              )}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="text-center py-12">
                      <p className="text-[#A7B4AD]">No checkpoints available for this course yet.</p>
                    </div>
                  )}
                </div>
              )}
                ...
                <div className="flex flex-col h-full">
                  <div className="flex-1 space-y-4 mb-4">
                    <div className="flex gap-3">
                      <div className="w-8 h-8 bg-[#2EE9A8] rounded-full flex items-center justify-center flex-shrink-0">
                        <Sparkles className="w-4 h-4 text-black" />
                      </div>
                      <div className="bg-[#0A0A0A] rounded-xl rounded-tl-none p-4 max-w-[80%]">
                        <p className="text-white">What is the time complexity of quicksort?</p>
                      </div>
                    </div>

                    <div className="flex gap-3 justify-end">
                      <div className="bg-[rgba(46,233,168,0.1)] border border-[rgba(46,233,168,0.3)] rounded-xl rounded-tr-none p-4 max-w-[80%]">
                        <p className="text-white">O(n log n) on average.</p>
                      </div>
                      <div className="w-8 h-8 bg-gradient-to-br from-[#2EE9A8] to-[#00CC33] rounded-full flex items-center justify-center flex-shrink-0">
                    ...
                    <div className="flex gap-3">
                      ...
                      <div className="bg-[#0A0A0A] rounded-xl rounded-tl-none p-4 max-w-[80%]">
                        <p className="text-white">Correct! Want a quick practice problem?</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 bg-slate-50 border border-slate-200 text-slate-900 px-4 py-3 rounded-xl focus:outline-none focus:border-emerald-500"
                    />
                    <MatrixButton>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className={cn(
                          ...
                          notesMode === 'edit'
                            ? 'bg-[#2EE9A8] text-black'
                            : 'bg-[#1F1F1F] text-[#A7B4AD]'
                        )}
                      >
                        Edit
                      </button>
